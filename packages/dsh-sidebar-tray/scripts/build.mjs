#!/usr/bin/env node
/**
 * Build the client bundle for dsh-sidebar-tray.
 *
 * The web shell loads client plugins through `window.__ModuleLoader__.load`,
 * where shared singletons (react and the client UI packages) arrive via the
 * injected `require` rather than being bundled. So: bundle our own sources,
 * but leave `react` external and route it through that `require`.
 */
...
const rawCssPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /\.css\?raw$/ }, (args) => {
      ...
      const path = bare.startsWith('.') ? join(args.resolveDir, bare) : join(root, '..', '..', 'node_modules', bare)
...
// The host half is plain ESM and needs no bundling step.
