...
var DEFAULT_SPACE_EMOJI = "\u{1F3E0}";
var SPACE_EMOJI_FALLBACK = "\u{1F4C1}";
var SPACE_EMOJI_PRESETS = ["\u{1F3E0}", "\u{1F4BC}", "\u{1F3AE}", "\u{1F9EA}", "\u{1F680}", "\u{1F3A8}", "\u{1F4DA}", "\u{1F4A1}", "\u{1F527}", "\u{1F9E0}", "\u{1F331}", "\u{1F525}", "\u26A1", "\u{1F3AF}", "\u{1F9E9}", "\u{1F6E0}\uFE0F", "\u{1F4C8}", "\u{1F3B5}", "\u{1F30A}", "\u{1F3D7}\uFE0F", "\u{1F419}", "\u{1F98A}", "\u{1F340}", "\u2B50"];
function normalizeEmoji(raw, fallback = SPACE_EMOJI_FALLBACK) {
  ...
  for (const ch of chars) {
    ...
    const joiner = code === 8205 || code >= 65024 && code <= 65039 || code >= 127995 && code <= 127999 || code >= 917536 && code <= 917631;
...
function normalizeDefaultSpace(raw) {
...
var DEFAULTS = {
  ...
  defaultSpace: { name: "Default", emoji: "\u{1F3E0}" }
};
...
function useChordHeld(React) {
  ...
  React.useEffect(() => {
    const down = (event) => {
      if (event.ctrlKey || event.metaKey) setHeld(true);
    };
    const up = (event) => {
      if (!(event.ctrlKey || event.metaKey)) setHeld(false);
    };
...
function SpaceHeader({ React, space, rail }) {
  return React.createElement(
    ...
    rail || space.workspaces.length === 0 ? null : React.createElement("span", { className: "tray-space-header-count" }, String(space.workspaces.length))
  );
}
function SpaceDock({ React, spaces, activeSpaceId, onSelect, onCreate, onRename, onEmoji, onDelete, onMove, rail }) {
  ...
  React.useEffect(() => {
    if (menuFor === null && emojiFor === null) return void 0;
    const close = () => {
      setMenuFor(null);
      setEmojiFor(null);
    };
  ...
  const items = spaces.map((space, index) => {
    ...
    return React.createElement(
      ...
      held && number <= 9 ? React.createElement("kbd", { className: "tray-space-hint", "aria-hidden": "true" }, `\u21E7${number}`) : null
    );
  });
  const menuSpace = menuFor === null ? void 0 : spaces.find((s) => s.id === menuFor);
  const emojiSpace = emojiFor === null ? void 0 : spaces.find((s) => s.id === emojiFor);
  const item = (label, onClick, extra = {}) => React.createElement("button", { type: "button", role: "menuitem", onClick, ...extra }, label);
  const menu = menuSpace === void 0 ? null : React.createElement(
    ...
    item("Change emoji\u2026", () => {
      setMenuFor(null);
      setEmojiFor(menuSpace.id);
    }),
    menuSpace.builtin ? null : item("Move left", () => {
      setMenuFor(null);
      onMove(menuSpace.id, -1);
    }),
    menuSpace.builtin ? null : item("Move right", () => {
      setMenuFor(null);
      onMove(menuSpace.id, 1);
    }),
    menuSpace.builtin ? null : item("Delete space", () => {
      setMenuFor(null);
  ...
  const emojiMenu = emojiSpace === void 0 ? null : React.createElement(
    "div",
