...
import { subscribeTerminalActions } from './terminal-actions.js'
...
export function TerminalPanel({ cwd }) {
  ...
  const addTab = useCallback((initialCommand) => {
  ...
  /* Accept a workspace action and route it to the focused PTY. */
  useEffect(() => {
      ...
      <div className="dshNtBody">
        {activeTabObject === null ? (
          ...
          <LayoutView
            ...
            renderPane={(paneId) => (
              <TerminalPane
                key={`${activeTabObject.id}:${paneId}:${cwd ?? ''}`}
                paneId={paneId}
                cwd={cwd}
                fontSize={fontSize}
