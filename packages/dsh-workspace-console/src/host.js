...
export const inject = ['subprocess', 'webServer', 'workspaceRegistry', 'connection']
...
export function apply(ctx, config) {
  ...
  async function handleHttp(req, res) {
    ...
    if (route === '/actions' && req.method === 'GET') {
      ...
      if (workspace.error) return json(res, workspace.status, { error: workspace.error })
      const result = readWorkspaceActions(workspace.value.path)
      return json(res, 200, result)
    }

    if (route === '/actions' && req.method === 'PUT') {
      const body = await readJson(req)
      const workspace = await resolveWorkspace(body?.cwd)
      if (workspace.error) return json(res, workspace.status, { error: workspace.error })
      const actions = validateWorkspaceActions(body?.actions)
      writeWorkspaceActions(workspace.value.path, actions)
      return json(res, 200, readWorkspaceActions(workspace.value.path))
    }
  ...
  async function resolveWorkspace(cwd) {
    if (!isAbsolute(cwd ?? '')) return { status: 400, error: 'cwd must be absolute' }
    const value = await ctx.workspaceRegistry.resolveByPath(cwd)
    if (value === undefined) return { status: 403, error: 'unknown workspace' }
    return { value }
  }

  /** Workspace-owned source of truth. This file is safe to commit with a project. */
  function actionsPath(cwd) {
    return resolvePath(cwd, '.agents', 'actions.json')
  }

  function readWorkspaceActions(cwd) {
    const file = actionsPath(cwd)
    try {
      let info
      try { info = statSync(file) } catch (error) {
        if (error?.code === 'ENOENT') return { actions: [], path: '.agents/actions.json' }
        throw error
      }
      if (!info.isFile()) return { actions: [], path: '.agents/actions.json' }
      if (info.size > 256 * 1024) throw new Error('actions file exceeds 256 KiB')
      const parsed = JSON.parse(readFileSync(file, 'utf8'))
      if (parsed?.version !== 1) throw new Error('actions file version must be 1')
      return { actions: validateWorkspaceActions(parsed?.actions), path: '.agents/actions.json' }
    } catch (error) {
      if (error?.code === 'ENOENT') return { actions: [], path: '.agents/actions.json' }
      throw new Error(`could not read ${file}: ${String(error?.message ?? error)}`)
    }
  }

  function validateWorkspaceActions(input) {
    if (input === undefined) return []
    if (!Array.isArray(input)) throw new Error('actions must be an array')
    if (input.length > 24) throw new Error('a workspace may define at most 24 actions')
    const ids = new Set()
    return input.map((raw, index) => {
      const name = boundedText(raw?.name, 'name', 1, 48)
      const command = boundedText(raw?.command, 'command', 1, 4096)
      const requestedId = typeof raw?.id === 'string' ? raw.id.trim() : ''
      const id = requestedId.length > 0 ? requestedId : slug(name)
      if (!/^[a-z0-9][a-z0-9-]{0,47}$/.test(id)) throw new Error(`action ${index + 1} has an invalid id`)
      if (ids.has(id)) throw new Error(`duplicate action id: ${id}`)
      ids.add(id)
      const shortcut = optionalText(raw?.shortcut, 64)
      const icon = optionalText(raw?.icon, 12) || 'run'
      return { id, name, command, icon, ...(shortcut ? { shortcut } : {}) }
    })
  }
  ...
  function optionalText(value, max) {
    ...
    if (typeof value !== 'string' || value.trim().length > max) throw new Error(`optional text must be at most ${max} characters`)
    return value.trim()
  }

  function writeWorkspaceActions(cwd, actions) {
    const file = actionsPath(cwd)
    const temp = `${file}.tmp-${process.pid}-${randomUUID()}`
    mkdirSync(dirname(file), { recursive: true })
    writeFileSync(temp, `${JSON.stringify({ version: 1, actions }, null, 2)}\n`, { mode: 0o600 })
    renameSync(temp, file)
  }

  function slug(value) {
    return String(value).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 48) || 'action'
  }
